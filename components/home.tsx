        ...
        if (availableRecipes === 0) {
          ...
          for (const recipe of detailedRecipes) {
            try {
              ...
              const result = await model.generateContent(prompt);
              const response = await result.response;
              recipe.instructions = response.text().replace(/\n/g, '<br>'); // Replace original instructions with processed ones
            } catch (err) {
              console.error('Error processing instructions for recipe:', recipe.id, err);
              recipe.instructions = `Error processing instructions: ${err.message}\n\nOriginal instructions: ${recipe.instructions}`;

              
              // Keep original instructions if processing fails
            }
          }
